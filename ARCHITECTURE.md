...
| `reltio.removeGitTenant` | Tree context (tenant, git mode only) | **"Remove Config"** — drops a single config from the git-sourced tree by filtering it out of the marker file, then re-derives naming for the rest. Clearing the last config clears state but keeps files on disk. |
