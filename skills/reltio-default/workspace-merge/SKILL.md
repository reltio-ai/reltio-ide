...
2. **Extension sync** — The extension refreshes `.reltio/reltio-agent/skills/default/**`, **`velocity-packs/**`**, and **`lca-knowledge-base/**`**. It **never** deletes or writes `skills/workspace/**`.
...
5. **Sibling playbooks** — Defaults include `entity-type-concepts`, `relation-type-concepts`, `attributes-from-concept`, `match-groups-from-concept`, **`lca-assistant`**, and this `workspace-merge` guide.
